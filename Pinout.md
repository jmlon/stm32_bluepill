...
| PC13 | Onboard LED | Hardwired to the user LED. Note that it is active-LOW (writing LOW turns it ON). Like PC14/PC15, it is limited to about 3 mA of drive — never use it for anything but a small LED. |
| PC14 & PC15 | Low-Speed Crystal (32.768 kHz) | Connected to the external RTC crystal. They have low current-drive capability (~3 mA) and should avoid heavy electrical loads. |
...
* The ADC-capable pins (PA0–PA7, PB0 and PB1) are NOT 5V tolerant. Connecting 5V to one will ruin it. They must stay under 3.3V.
* Outputs cannot source 5V. Even on 5V-tolerant pins, a high digital output will only output 3.3V. [1, 5]

### 🔌 Current Limitations

* Per-pin maximum: Individual pins can safely source or sink up to 25 mA (except PC13–PC15, see above).
...
### 🔀 EXTI Lines Are Shared

Any GPIO pin can be used as an external interrupt source, but there are only 16 EXTI lines and they are shared by pin *number*: PA0, PB0 and PC0 all map to EXTI0. Only one pin per number can be an active interrupt source at a time, so plan interrupt-driven inputs across different pin numbers.

### 🔁 USB and CAN Cannot Be Used Together

The USB and CAN peripherals share the same 512-byte SRAM buffer inside the chip. Only one of the two can be active at any given time.
...
* PWM Capability: The four timers provide 16 capture/compare channels in total. The number usable at once is lower in practice, because the channel pins overlap with USB, the crystals, and other peripherals.
...
* SPI (Serial Peripheral Interface) x 2: Two high-speed SPI ports for fast displays (like TFT screens) and SD card modules. SPI1 sits on the 72 MHz APB2 bus and reaches 18 Mbit/s; SPI2 is on the 36 MHz APB1 bus and tops out at 9 Mbit/s.
...
* CAN Bus 2.0B x 1: An automotive-grade Controller Area Network interface. It requires an external transceiver chip (like the MCP2551) to connect to a physical CAN network but handles all protocol framing internally. It cannot be used at the same time as USB.

### 4. Direct Memory Access (DMA)

The STM32F103C8T6 has a single DMA controller (DMA1) with 7 channels, and it is one of the Blue Pill's greatest performance advantages. It allows peripherals (like the ADC, SPI, or UART) to send and receive data directly to and from the chip's internal RAM without involving the CPU. For example, the ADC can stream thousands of sensor readings into a data array in the background while the CPU performs heavy math calculations.
...
* Nested Vectored Interrupt Controller (NVIC): Manages low-latency real-time interrupts. Any GPIO pin can be configured as an external interrupt source, subject to the shared-EXTI-line limit described above.
...
1. The Onboard Power LED: The board has a red power LED hardwired directly to the 3.3V line. This single LED continuously draws about 3 to 5 mA. If you want true microamp consumption, you must remove it (usually labeled PWR) — desolder it, or lift one end of its series resistor.
...
1. [Blue Pill 3.3V architecture and I/O limits — Mecrisp discussion](https://sourceforge.net/p/mecrisp/discussion/general/thread/47921c0c90/)
2. [Push button with STM32 Blue Pill in STM32CubeIDE — microcontrollerslab.com](https://microcontrollerslab.com/push-button-stm32-blue-pill-stm32cube-ide-tutorial/)
3. [Basic questions about alternate functions — ST Community](https://community.st.com/stm32-mcus-products-25/basic-questions-about-alternate-functions-100663)
4. [Recommended course of action for unused pins — ST Community](https://community.st.com/stm32-mcus-products-25/recommended-course-of-action-for-unused-pin-of-stm32-mcu-119796)
5. [Introduction to the STM32 Blue Pill — robocraze.com](https://robocraze.com/blogs/post/introduction-to-the-stm32-bluepill)
6. [Which pins are usable for plain I/O — ST Community](https://community.st.com/stm32-mcus-products-25/stm32446re-is-there-a-simple-chart-that-states-which-pins-are-usable-for-just-i-o-120611)
7. [STM32F103C8T6 Blue Pill articles — makeriot2020.com](https://www.makeriot2020.com/index.php/category/stm32f103c8t6-blue-pill/)
8. [Arduino on the STM32 Blue Pill — VisualGDB tutorials](https://visualgdb.com/tutorials/arduino/stm32/bluepill/)
9. [Blue Pill counterfeit chip walkthrough — YouTube](https://www.youtube.com/watch?v=L670v-Oghs4)
